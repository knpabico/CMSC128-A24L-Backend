    ...
    <div className='bg-[#EAEAEA] h-full'>
		...
		<div className='my-[40px] mx-[30px] h-fit flex flex-col gap-[40px] md:flex-row lg:mx-[100px] xl:mx-[200px]'>
			<div className='bg-[#FFFFFF] flex flex-col p-7 gap-[10px] rounded-[10px] w-content h-max'>
				<button className={`flex gap-5 items-center ${filterType === 'all' ? 'bg-blue-100' : 'bg-gray-200'}`} onClick={() => setFilterType('all')}>
					<CalendarDays />
				...
				<button className='flex gap-5 items-center' onClick={() => setFilterType('bookmarked')}>
					<Bookmark />
					<p className="group relative w-max">
			...
			<div className='flex flex-col gap-[10px]  w-full mb-10'>
				...
				<div className="flex flex-col md:flex-row justify-between mb-6 space-y-4 md:space-y-0">					
					...
					<button className={`px-4 py-2 rounded-lg ${sortOrder === 'latest' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`} onClick={() => setSortOrder('latest')}>
						Latest First
					</button>
					<button className={`px-4 py-2 rounded-lg ${sortOrder === 'oldest' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`} onClick={() => setSortOrder('oldest')} >
						Oldest First
					</button>
					</div>
				</div>
