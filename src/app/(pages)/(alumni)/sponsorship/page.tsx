...
const SponsorshipPage: React.FC = () => {
  ...
  const { sponsorships, loading, error, refreshSponsorships } =
    useSponsorships();
  ...
  useEffect(() => {
    if (activeFilter === "all") {
      ...
      setFilteredSponsorships(
        sponsorships.filter(
          (s: { status: string }) => s.status === activeFilter
        )
      );
    }
        ...
        <div className="flex flex-col gap-[10px]  w-full mb-10">
          ...
          <div className="bg-[#FFFFFF] rounded-[10px] flex justify-around">
            <button className="mr-2" onClick={() => handleFilterChange("all")}>
              <p
                className={`relative px-4 py-2 w-full transition-all ${
                  activeFilter === "all"
          ...
          {filteredSponsorships.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-lg w-full">
              <h3 className="text-xl font-medium text-gray-600">
                No donation drive found
              </h3>
              <p className="text-gray-500 mt-2">
                There are no donation drive with the selected filter.
              </p>
            </div>
          ) : (
              ...
              {filteredSponsorships.map((sponsorship) => (
                <div key={sponsorship.sponsorshipId} className="...">
                  <div className=" bg-[#FFFF] rounded-[10px] h-fit">
                    {/* Image */}
                    <div
                      className="bg-cover bg-center rounded-t-[10px] h-[230px]"
                    ...
                    <div className="p-[30px]">
                      ...
                      <p className="text-sm mb-2">
                        ...
                        · {sponsorship.creatorType}
                      </p>
                      {/* Description */}
                      <p className="mb-5 text-sm h-max">
                      ...
                      <div className="my-5">
                          ...
                          <div
                            ...
                            style={{
                              width: `${calculateProgress(
                                sponsorship.currentAmount,
                                sponsorship.targetAmount
                              )}%`,
                            }}
                          ></div>
                        </div>
                        <div className="flex justify-between my-1 text-sm">
                          <span className="font-medium">
                            ${sponsorship.currentAmount.toLocaleString()}
                          </span>
                          <span className="text-gray-500">
                            of ${sponsorship.targetAmount.toLocaleString()}
                          </span>
                        </div>
                      </div>
                      ...
                      <button
                        className="flex gap-2 items-center group relative w-max"
                        onClick={() =>
                          handleViewDetails(sponsorship.sponsorshipId)
                        }
                      >
