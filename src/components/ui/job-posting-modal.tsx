...
function JobPostingContent() {
  ...
  useEffect(() => {
    if (
      jobOffers
        .filter(
          (jobOffer: JobOffering) => jobOffer.alumniId == alumInfo?.alumniId
        )
        .filter((jobOffer: JobOffering) => jobOffer.status !== "Draft").length >
        0 &&
      selectedJob
    ) {
      setSelectedJob(null);
  ...
  useEffect(() => {
    ...
    const sorted = [...filteredJobs].sort((a, b) => {
      const aTime = a.datePosted?.toMillis
        ? a.datePosted.toMillis()
        : new Date(a.datePosted).getTime() || 0;
      const bTime = b.datePosted?.toMillis
        ? b.datePosted.toMillis()
    ...
    const sortedApp = [...jobApplications].sort((a, b) => {
      const aTime = a.dateApplied?.toMillis
        ? a.dateApplied.toMillis()
        : new Date(a.dateApplied).getTime() || 0;
      const bTime = b.dateApplied?.toMillis
        ? b.dateApplied.toMillis()
        : new Date(b.dateApplied).getTime() || 0;
              ...
              {sortedJobs
                .filter(
                  (jobOffer: JobOffering) =>
                    jobOffer.alumniId == alumInfo?.alumniId
                )
                .filter((jobOffer: JobOffering) =>
                  sortStatus !== "all"
                    ? jobOffer.status ===
                      sortStatus.charAt(0).toUpperCase() + sortStatus.slice(1)
                            ...
                            onClick={(e) => {
                              ...
                              handleDelete(jobOffer.jobId);
                              {
                                (selectedJob && selectedJob.jobId) ===
                                  jobOffer.jobId && setSelectedJob(null);
                              }
                            }}
                          >
              ...
              {selectedJob !== null && (
                <div className="p-5">
                  <div className="flex justify-between mb-7">
                    ...
                    <XIcon
                      className="text-gray-700 hover:text-red-500 cursor-pointer w-6 h-6"
                  ...
                  <div className="space-y-2">
                    ...
                    {/* Settings Option */}
                    <Collapsible
                      open={isApplicantsOpen}
                      onOpenChange={setIsApplicantsOpen}
                    >
                      <div className="flex items-center">
                        <Button
                          variant="ghost"
                            ...
                            <div className="space-x-2">
                              <Button
                                className={`cursor-pointer ${
                                  pending
                                    ? `bg-amber-200`
                                    : `text-gray-500 font-light border border-gray-200`
                                }`}
              ...
              {sortedApps.filter((offer: JobApplication) =>
                sortStatus !== "all"
                  ? offer.applicantId === user!.uid &&
                    offer.status === sortStatus
                  : offer.applicantId === user!.uid
              ).length === 0 ? (
                <div className="flex flex-col p-5 max-h-fit space-y-1 w-full justify-center items-center">
                  <p className="text-gray-700">
                    No {sortStatus} applications yet.
                  </p>
                </div>
              ) : (
                <div>
                  {sortedApps
                    .filter((offer: JobApplication) =>
                      sortStatus !== "all"
                        ? offer.applicantId === user!.uid &&
                          offer.status === sortStatus
                        : offer.applicantId === user!.uid
          ...
          <div className="flex items-start w-full min-w-0">
            ...
            >
              {sortedJobs
                ...
                .filter((jobOffer: JobOffering) => jobOffer.status === "Draft")
                .length > 0 ? (
                sortedJobs
                  .filter(
                    (jobOffer: JobOffering) =>
                  ...
                  .filter(
                    (jobOffer: JobOffering) => jobOffer.status === "Draft"
                  )
                  .map((jobOffer: JobOffering) => (
                    <div
                      key={jobOffer.jobId}
